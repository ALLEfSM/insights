...
public class AuthController {
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "204", description = "login realizado com secesso",
                    content = @Content(mediaType = "application/json",schema = @Schema(implementation = DetalhesCliente.class))),
    ...
    public ResponseEntity login(@RequestBody LoginRequestDTO body){
        Cliente cliente =  this.clienteRepository.findByLogin(body.login()).orElseThrow(() -> new RuntimeException("Cliente não encontrado"));
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "204", description = "Registro realizado com secesso",
                    content = @Content(mediaType = "application/json",schema = @Schema(implementation = DetalhesCliente.class))),
            @ApiResponse(responseCode = "400", description = "Dados inválidos"),
            @ApiResponse(responseCode = "404", description = "informações não encontrado")}
    )
